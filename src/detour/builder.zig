...
/// Off-mesh classification — split out of createNavMeshData with `noinline` so
/// the (cold for tile-cache rebuilds, which never carry off-mesh connections)
/// height-scan + classify loops do not bloat the hot body. Behaviour byte-equal.
noinline fn classifyOffMeshConnections(
    params: *const NavMeshCreateParams,
    off_mesh_con_class: []u8,
) struct { stored: usize, links: usize } {
    var stored: usize = 0;
    var links: usize = 0;
    ...
    for (0..params.off_mesh_con_count) |i| {
        ...
        // Count links
        if (off_mesh_con_class[i * 2 + 0] == 0xff) links += 1;
        if (off_mesh_con_class[i * 2 + 1] == 0xff) links += 1;

        if (off_mesh_con_class[i * 2 + 0] == 0xff) stored += 1;
    }

    return .{ .stored = stored, .links = links };
}

/// Off-mesh vertex/polygon/connection stores — same `noinline` cold-split as
/// classifyOffMeshConnections (loops are 0-trip on the tile-cache path but their
/// inline bodies cost hot-body I-cache). Verbatim moves; behaviour byte-equal.
noinline fn storeOffMeshVerts(
    params: *const NavMeshCreateParams,
    off_mesh_con_class: []const u8,
    nav_verts: [*]f32,
    off_mesh_verts_base: usize,
) void {
    var n: usize = 0;
    for (0..params.off_mesh_con_count) |i| {
        if (off_mesh_con_class[i * 2 + 0] == 0xff) {
...
noinline fn storeOffMeshPolys(
    params: *const NavMeshCreateParams,
    off_mesh_con_class: []const u8,
    nav_polys: [*]Poly,
    off_mesh_poly_base: usize,
    off_mesh_verts_base: usize,
) void {
    var n: usize = 0;
    for (0..params.off_mesh_con_count) |i| {
...
pub fn createNavMeshData(
    ...
    if (params.off_mesh_con_count > 0) {
        ...
        const counts = classifyOffMeshConnections(params, off_mesh_con_class);
        stored_off_mesh_con_count = counts.stored;
        off_mesh_con_link_count = counts.links;
    }
    // NOTE: the free used to be a `defer` INSIDE the block above, which frees the
    // heap branch (>512 connections) before the store loops below read it — a
    // latent use-after-free. Function-scope defer matches C++ (freed at the end).
    ...
    // Off-mesh connection vertices (cold-split, no-op when count == 0)
    if (params.off_mesh_con_count > 0) {
        storeOffMeshVerts(params, off_mesh_con_class, nav_verts, off_mesh_verts_base);
    }
    ...
    // Off-mesh connection polygons (cold-split, no-op when count == 0)
    if (params.off_mesh_con_count > 0) {
        storeOffMeshPolys(params, off_mesh_con_class, nav_polys, off_mesh_poly_base, off_mesh_verts_base);
    }
    ...
    // Store off-mesh connections (cold-split, no-op when count == 0)
    if (params.off_mesh_con_count > 0) {
        storeOffMeshCons(params, off_mesh_con_class, off_mesh_cons, off_mesh_poly_base);
    }
