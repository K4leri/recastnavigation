//! Complete, runnable end-to-end pipeline: build a navmesh from raw triangles and
//! find a path across it. Mirrors the API used by the integration tests
//! (`test/integration/detour_pipeline_test.zig`, `pathfinding_test.zig`), which are
//! the living, CI-verified reference for how to drive the library.
//!
...
pub fn main() !void {
    // DebugAllocator (the 0.16 successor to GeneralPurposeAllocator): the example
    // doubles as a leak smoke-test — deinit() reports whether anything was left
    // unfreed. In production code you'd swap this for std.heap.smp_allocator.
    ...
    std.debug.print("Full navmesh build & pathfinding\n", .{});
    std.debug.print("================================\n\n", .{});

    // ---------------------------------------------------------------------
    // 1. Input geometry: an L-shaped floor (flat f32 xyz, i32 triangle indices)
    // ---------------------------------------------------------------------
    const verts = [_]f32{
        0,  0, 0, // 0
        20, 0, 0, // 1
    ...
    // ---------------------------------------------------------------------
    // 9. Query: nearest polys -> findPath -> findStraightPath
    // ---------------------------------------------------------------------
    ...
    const end_in = [3]f32{ 28.0, 0.0, 8.0 };
